...
switch (playerSide) do {
	case civilian: {
		...
		//suicide vests
		life_actions = life_actions + [player addAction["Activate Suicide Vest",life_fnc_suicideVest,"",0,false,false,"",'vest player == "V_Press_F" && alive player && playerSide == civilian && !life_istazed && !(player getVariable "restrained") && !(player getVariable "Escorting") && !(player getVariable "transporting")']];
	};
};