...
/*
if((nearestObject [[6018.1,10171.9,0],"Land_Offices_01_V1_F"]) == _building && ({side _x == west} count playableUnits < (LIFE_SETTINGS(getNumber,"cops_online_min")))) exitWith {
	hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"cops_online_min"))]
};
if((typeOf _building) == "Land_Offices_01_V1_F" && life_fed_break == 0) exitWith {hint localize "STR_ISTR_Bolt_Exploit"};
*/
if(isNil "life_boltcutter_uses") then {life_boltcutter_uses = 0;};

/*
if((nearestObject [[6018.1,10171.9,0],"Land_Offices_01_V1_F"]) == _building) then {
...
switch (typeOf _building) do {
	case "Land_Offices_01_V1_F": {if(life_fed_break == 0) then {life_fed_break = 2;};};
};
