...
switch (_code) do {
	...
	case 19: {
		...
		if(_shift && playerSide in [west,civilian] && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget GVAR "Escorting") && !(cursorTarget GVAR "restrained") && speed cursorTarget < 1) then {
			if(playerSide == civilian) then {
				if((animationState cursorTarget) == "Incapacitated") then {
					//if([false,"handcuffs",1] call life_fnc_handleInv) then {
						//[] call life_fnc_restrainAction;
						//hint localize "STR_NOTF_RestrainedPerson";
					//};
				} else {
					if((currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player GVAR ["restrained",false]) && !life_istazed && !(player GVAR ["surrender",false])) then {
	...
	case 24: {
		if(_shift) then {
			if (soundVolume != 1) then {
				1 fadeSound 1;
				hint "You have taken out your ear plugs.";
			} else {
				1 fadeSound 0.1;
				hint "You have inserted your ear plugs.";
			};
		};
